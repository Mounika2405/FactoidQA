...
from .scripts import load_data_generators, get_data, get_answer_tokens
from .scripts import data_download_and_preprocess, negative_avg_log_error, accuracy, compute_accuracy, compute_EM, compute_f1
import os
import numpy as np
import pickle
...
parser.add_argument('-e', '--emdim',  choices=[350, 400, 500, 600, 700],
...
def main():
    ...
    if args.which == 'train':
        ...
        if args.model_name is not None:
            ...
            print("Loaded Bidaf model")
            bidaf_model.model.compile(loss=negative_avg_log_error, optimizer='adadelta', metrics=[accuracy])

        vectors, contexts, questions, answers = get_data('train', emdim=args.emdim, squad_version=args.squad_version,
                                                        max_passage_length=args.max_passage_length,
                                                        max_query_length=args.max_query_length)


        train_generator, validation_generator = load_data_generators(vectors, contexts, questions, answers,
                                                                    batch_size=args.batch_size, emdim=args.emdim, 
    ...
    if args.which == 'predict':
        ...
        vectors, contexts, questions, answers = get_data('dev', emdim=args.emdim, squad_version=args.squad_version,
                                                        max_passage_length=args.max_passage_length,
                                                        max_query_length=args.max_query_length)
        ...
        for i in range(len(contexts)):
            ...
            answer = bidaf_model.predict_ans(vectors, contexts[i], questions[i], squad_version=args.squad_version,
        ...
        with open('what_type_predictions.pickle', 'wb') as file:
            pickle.dump(answer_pred, file, protocol=pickle.HIGHEST_PROTOCOL)

        

        answer_tokens = get_answer_tokens(squad_version=args.squad_version)

        em = []
        f1 = []
        for i in range(len(answer_pred)):
            exact_match = compute_EM(answer_tokens[i], answer_pred[i])
            em.append(exact_match)

            score = compute_f1(answer_tokens[i], answer_pred[i])
            f1.append(score)

        em_score = np.sum(em)/len(em)
        f1_score = np.sum(f1)/len(f1)
